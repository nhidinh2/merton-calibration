...
def main():
    ...
    2. Aligns debt to daily (forward-fill quarterly values)
    3. For each firm and date, calibrates asset value and volatility
    4. Computes risk measures (DD, PD)
    ...
    for firm_id in equity_prices['firm_id'].unique():
        ...
        firm_vol = equity_vol[equity_vol['firm_id'] == firm_id].sort_values('date')
        ...
        for date in firm_equity['date']:
            ...
            sigma_E = vol_row['equity_vol'].values[0]
            D = debt_row['debt'].values[0]
            r = rf_row['risk_free_rate'].values[0]
            
            # Skip if any value is invalid
            if pd.isna(E_per_share) or pd.isna(sigma_E) or pd.isna(D) or pd.isna(r):
                continue
            if E_per_share <= 0 or sigma_E <= 0 or D <= 0 or r < 0:
                continue
            ...
            try:
                ...
                V, sigma_V = calibrate_asset_parameters(E, sigma_E, D, T, r, V0=V0, sigma_V0=sigma_V0)
    ...
    output_file = output_dir / 'naive_results.csv'

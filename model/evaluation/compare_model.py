"""
Model Comparison Script - Baseline vs Improved

Compares baseline (naive) and improved Merton model implementations:
...
Loads both baseline and improved results for side-by-side comparison.
"""
...
from improved.calibration import calibrate_asset_parameters as calibrate_improved
from improved.risk_measures import compute_risk_measures as compute_risk_improved
from improved.smoothing import smooth_equity_volatility

# Import naive functions for comparison
try:
    from naive_model.calibration import calibrate_asset_parameters as calibrate_naive
    from naive_model.risk_measures import compute_risk_measures as compute_risk_naive
    NAIVE_AVAILABLE = True
except ImportError:
    NAIVE_AVAILABLE = False
    calibrate_naive = None
    compute_risk_naive = None


# ----------------------------- Load + merge ----------------------------- #

def load_naive_results_and_inputs():
    """
    Load naive (baseline) model results and merge with input data for comparison.
    """
    ...
    # Merge inputs (naive uses raw volatility, not smoothed)
...
def load_results_and_inputs():
    """
    Load improved model results and merge with input data.
    
    Uses the same data loading and alignment logic as improved/__main__.py
    to ensure consistency with the results being compared.
    """
    # Load all data
    project_root = Path(__file__).parent.parent.parent
    data_dir = project_root / 'data' / 'real'
    ...
    results = pd.read_csv(out_dir / 'improved_results.csv', parse_dates=['date'])
    equity_prices = pd.read_csv(data_dir / 'equity_prices.csv', parse_dates=['date'])
    equity_vol = pd.read_csv(data_dir / 'equity_vol.csv', parse_dates=['date'])
    debt = pd.read_csv(data_dir / 'debt_quarterly.csv', parse_dates=['date'])
    risk_free = pd.read_csv(data_dir / 'risk_free.csv', parse_dates=['date'])
    ...
    # Smooth equity volatility (same as in improved/__main__.py)
...
def diagnose_implausible_outputs(results):
    """Check for economically implausible V values."""
    df = results.copy()
    n_V_invalid = ((df["V"] <= 0) | (~np.isfinite(df["V"]))).sum()
    
    # Also check imputed flag if available
    n_imputed = 0
    ...
    return {
        ...
        "n_imputed": int(n_imputed) if "imputed" in df.columns else None
    }
...
def diagnose_sensitivity(results, seed=42, use_naive=False):
    """
    ...
    Args:
        ...
        use_naive: If True, use naive calibration functions; otherwise use improved
    """
    df = results.dropna(subset=["E", "sigma_E", "D", "r"]).copy()
    if len(df) == 0:
        return {"note": "No rows with complete inputs"}

    # Select calibration functions based on model type
    if use_naive and NAIVE_AVAILABLE:
        calibrate_fn = calibrate_naive
        compute_risk_fn = compute_risk_naive
    else:
        calibrate_fn = calibrate_improved
        compute_risk_fn = compute_risk_improved
    ...
    for _, row in samp.iterrows():
        ...
        for name, (_, size) in pert.items():
            ...
            try:
                ...
                V1, sV1 = calibrate_fn(E1, sE1, D1, T1, r1, V0=V0, sigma_V0=sigma_V0)
                out = compute_risk_fn(V1, D1, T1, r1, sV1)
...
def plot_diagnostics(results, diagnostics, output_dir=None):
    ...
    for idx, firm_id in enumerate(firms_to_plot):
        ...
        # Mark imputed points if available
        if 'imputed' in firm_data.columns:
            imputed_data = firm_data[firm_data['imputed'] == True]
            non_imputed_data = firm_data[firm_data['imputed'] == False]
            
            ax.plot(non_imputed_data['date'], non_imputed_data['log_PD'], 'b-', 
                   alpha=0.7, label='log(PD)', linewidth=1.5)
            if len(imputed_data) > 0:
                ax.scatter(imputed_data['date'], imputed_data['log_PD'], 
                          color='red', marker='x', s=30, alpha=0.6, label='Imputed')
        else:
            ax.plot(firm_data['date'], firm_data['log_PD'], 'b-', alpha=0.7, 
                   label='log(PD)', linewidth=1.5)
    ...
    plt.tight_layout()
    plt.savefig(output_dir / 'diagnosis_timeseries_improved.png', dpi=150, bbox_inches='tight')
    print(f"Saved PD time series plot to {output_dir / 'diagnosis_timeseries_improved.png'}")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(output_dir / 'diagnosis_v_e_ratio_improved.png', dpi=150, bbox_inches='tight')
    print(f"Saved V/E ratio plot to {output_dir / 'diagnosis_v_e_ratio_improved.png'}")
    plt.close()
    ...
    if 'ranking' in diagnostics and 'daily_results' in diagnostics['ranking']:
        ...
        if firm_rank_trajectories:
            ...
            plt.tight_layout()
            plt.savefig(output_dir / 'diagnosis_ranking_improved.png', dpi=150, bbox_inches='tight')
            print(f"Saved ranking plot to {output_dir / 'diagnosis_ranking_improved.png'}")
            plt.close()
    ...
    if 'sensitivity' in diagnostics and 'stats' in diagnostics['sensitivity']:
        ...
        if sens_stats:
            ...
            plt.tight_layout()
            plt.savefig(output_dir / 'diagnosis_sensitivity_improved.png', dpi=150, bbox_inches='tight')
            print(f"Saved sensitivity plot to {output_dir / 'diagnosis_sensitivity_improved.png'}")
            plt.close()


# ----------------------------- Part 6 Metrics ----------------------------- #

def calculate_part6_metrics(improved_results, naive_results=None):
    """
    Calculate all Part 6 metrics for improved model evaluation.
    
    Returns a dictionary with Part 6 formatted metrics ready for comparison.
    """
    # Calculate all diagnostics for improved model
    d1_improved = diagnose_unstable_signals(improved_results)
    d2_improved = diagnose_implausible_outputs(improved_results)
    d3_improved = diagnose_risk_ranking(improved_results)
    d4_improved = diagnose_sensitivity(improved_results, use_naive=False)
    
    part6_metrics = {
        'improved': {
            'pd_stability': d1_improved,
            'asset_plausibility': d2_improved,
            'risk_ranking': d3_improved,
            'sensitivity': d4_improved
        }
    }
    
    # If naive results available, calculate for comparison
    if naive_results is not None:
        d1_naive = diagnose_unstable_signals(naive_results)
        d2_naive = diagnose_implausible_outputs(naive_results)
        d3_naive = diagnose_risk_ranking(naive_results)
        d4_naive = diagnose_sensitivity(naive_results, use_naive=True)
        
        part6_metrics['naive'] = {
            'pd_stability': d1_naive,
            'asset_plausibility': d2_naive,
            'risk_ranking': d3_naive,
            'sensitivity': d4_naive
        }
    
    return part6_metrics


def print_part6_summary(part6_metrics):
    """Print Part 6 formatted summary tables."""
    print("\n" + "=" * 72)
    print("PART 6: IMPROVED MODEL RESULTS - QUANTITATIVE COMPARISON")
    print("=" * 72)
    
    improved = part6_metrics['improved']
    naive = part6_metrics.get('naive', None)
    
    # (1) PD Stability
    print("\n### (1) PD Stability")
    print("-" * 72)
    if "max_abs_dlogPD_per_firm" in improved['pd_stability']:
        print("\nMaximum daily change in log(PD) per firm:")
        print(f"{'Firm':<8} {'Improved max |Δlog(PD)|':<25}", end="")
        if naive and "max_abs_dlogPD_per_firm" in naive['pd_stability']:
            print(f"{'Baseline max |Δlog(PD)|':<25}")
            print("-" * 58)
            
            # Create comparison table
            improved_dict = {x['firm_id']: x['max_abs_dlogPD'] 
                           for x in improved['pd_stability']['max_abs_dlogPD_per_firm']}
            naive_dict = {x['firm_id']: x['max_abs_dlogPD'] 
                          for x in naive['pd_stability']['max_abs_dlogPD_per_firm']}
            
            all_firms = sorted(set(list(improved_dict.keys()) + list(naive_dict.keys())))
            for firm_id in all_firms:
                imp_val = improved_dict.get(firm_id, None)
                nav_val = naive_dict.get(firm_id, None)
                if nav_val is not None and imp_val is not None:
                    print(f"{firm_id:<8} {imp_val:<25.4f} {nav_val:<25.4f}")
                elif imp_val is not None:
                    print(f"{firm_id:<8} {imp_val:<25.4f} {'N/A':<25}")
        else:
            print()
            print("-" * 33)
            for firm_data in improved['pd_stability']['max_abs_dlogPD_per_firm']:
                print(f"{firm_data['firm_id']:<8} {firm_data['max_abs_dlogPD']:<25.4f}")
    
    # (2) Asset Value Plausibility - just report invalid V count
    print("\n### (2) Asset Value Plausibility")
    print("-" * 72)
    print(f"Invalid V values: {improved['asset_plausibility']['n_V_invalid']}")
    if improved['asset_plausibility'].get('n_imputed') is not None:
        print(f"Imputed observations: {improved['asset_plausibility']['n_imputed']}")
    if naive:
        print(f"Baseline invalid V values: {naive['asset_plausibility']['n_V_invalid']}")
    
    # (3) Risk Ranking Consistency
    print("\n### (3) Risk Ranking Consistency")
    print("-" * 72)
    if "spearman_rho_median" in improved['risk_ranking']:
        print(f"Distress proxy: {improved['risk_ranking'].get('distress_proxy', 'leverage')}")
        print(f"Number of days analyzed: {improved['risk_ranking']['n_dates']}")
        print(f"\n{'Metric':<40} {'Improved':<20}", end="")
        if naive and "spearman_rho_median" in naive['risk_ranking']:
            print(f"{'Baseline':<20}")
            print("-" * 80)
            
            imp_wrong = improved['risk_ranking']['wrong_sign_pct']
            nav_wrong = naive['risk_ranking']['wrong_sign_pct']
            
            imp_top1 = improved['risk_ranking']['top1_not_in_top2_PD_pct']
            nav_top1 = naive['risk_ranking']['top1_not_in_top2_PD_pct']
            
            print(f"{'Median Spearman ρ_t':<40} {improved['risk_ranking']['spearman_rho_median']:<20.3f} "
                  f"{naive['risk_ranking']['spearman_rho_median']:<20.3f}")
            print(f"{'% of days with wrong sign':<40} {imp_wrong:<20.2f}% {nav_wrong:<20.2f}%")
            print(f"{'Top-1 distress not in top-2 PD %':<40} {imp_top1:<20.2f}% {nav_top1:<20.2f}%")
        else:
            print()
            print("-" * 60)
            print(f"{'Median Spearman ρ_t':<40} {improved['risk_ranking']['spearman_rho_median']:<20.3f}")
            print(f"{'% of days with wrong sign':<40} {improved['risk_ranking']['wrong_sign_pct']:<20.2f}%")
            print(f"{'Top-1 distress not in top-2 PD %':<40} {improved['risk_ranking']['top1_not_in_top2_PD_pct']:<20.2f}%")
    
    # (4) Sensitivity to Inputs
    print("\n### (4) Sensitivity to Inputs")
    print("-" * 72)
    if "stats" in improved['sensitivity']:
        print(f"\n{'Parameter':<12} {'Improved median |sens|':<25} {'Improved p95 |sens|':<25}", end="")
        if naive and "stats" in naive['sensitivity']:
            print(f"{'Baseline median |sens|':<25} {'Baseline p95 |sens|':<25}")
            print("-" * 97)
            
            for param in sorted(set(list(improved['sensitivity']['stats'].keys()) + 
                                   list(naive['sensitivity']['stats'].keys()))):
                imp_stat = improved['sensitivity']['stats'].get(param, {})
                nav_stat = naive['sensitivity']['stats'].get(param, {})
                
                imp_med = imp_stat.get('median_abs', None)
                imp_p95 = imp_stat.get('p95_abs', None)
                nav_med = nav_stat.get('median_abs', None)
                nav_p95 = nav_stat.get('p95_abs', None)
                
                if nav_med is not None and imp_med is not None:
                    print(f"{param:<12} {imp_med:<25.3f} {imp_p95:<25.3f} "
                          f"{nav_med:<25.3f} {nav_p95:<25.3f}")
                elif imp_med is not None:
                    print(f"{param:<12} {imp_med:<25.3f} {imp_p95:<25.3f} {'N/A':<25} {'N/A':<25}")
        else:
            print()
            print("-" * 62)
            for param, stat in improved['sensitivity']['stats'].items():
                print(f"{param:<12} {stat['median_abs']:<25.3f} {stat['p95_abs']:<25.3f}")
...
def main():
    """Run all diagnostic checks and Part 6 metrics."""
    print("=" * 72)
    ...
    print(f"Loaded improved model results: {len(improved_results):,} rows")
    
    # Try to load naive results for comparison
    naive_results = load_naive_results_and_inputs()
    if naive_results is not None:
        print(f"Loaded baseline (naive) model results: {len(naive_results):,} rows")
    else:
        print("Baseline (naive) results not found - will calculate improved metrics only")

    # Calculate Part 6 metrics
    part6_metrics = calculate_part6_metrics(improved_results, naive_results)
    
    # Prepare diagnostics dict for plotting (using improved results)
    diagnostics = {
        "unstable": part6_metrics['improved']['pd_stability'],
        "implausible": part6_metrics['improved']['asset_plausibility'],
        "ranking": part6_metrics['improved']['risk_ranking'],
        "sensitivity": part6_metrics['improved']['sensitivity']
    }
    
    # Generate plots
    print("\n--- Generating Plots ---")
    plot_diagnostics(improved_results, diagnostics)
    
    # Print Part 6 summary
    print_part6_summary(part6_metrics)
